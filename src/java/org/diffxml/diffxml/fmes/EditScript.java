...
public final class EditScript {
  ...
  /**
   * Constructor for EditScript. Used to create a list of modifications that
   * will turn doc1 into doc2, given a set of matching nodes.
   * 
   * @param doc1
   *          the original document
   * @param doc2
  ...
  /**
   * Creates an Edit Script conforming to matchings that transforms doc1 into
   * doc2.
   *
   ...
   *           When the output doc can't be made
   */
  public Document create() throws DocumentCreationException {
    ...
    try {
      ...
      throw new DocumentCreationException("Failed to create edit script", e);
    }
    ...
    while (!fifo.isEmpty()) {
      ...
      if (!mMatchings.isMatched(x)) {
        ...
        if (NodeOps.checkIfSameNode(x, doc2docEl)
          ...
          w = doUpdate(w, x);
        } else if (!mMatchings.getPartner(y).equals(w.getParentNode())) {
  ...
  /**
   * Inserts (the import of) node x as child of z according to the algorithm and
   * updates the Edit Script.
   *
   * @param x
   *          current node
   * @param z
   *          partner of x's parent
  ...
  /**
   * Performs a move operation according to the algorithm and updates the
   * EditScript.
   *
   * @param w
  ...
  private void deletePhase(final Node n, final NodePairs matchings) {
    ...
    if (!matchings.isMatched(n) && n.getNodeType() != Node.DOCUMENT_TYPE_NODE) {
      mDelta.delete(n);
   ...
   *
   * @param n
   *          the parent of the nodes to mark out of order
   */
   ...
   *
   * @param n
   *          the parent of the nodes to mark in order
   */
   ...
   *
   * @param w
   *          Node with potentially misaligned children
   * @param wSeq
   *          Sequence of children of w that have matches in the children of x
   * @param stay
   *          The List of nodes not to be moved
   * @param matchings
   *          The set of matching nodes
   */
